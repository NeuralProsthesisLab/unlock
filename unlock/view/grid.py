...
class HierarchyGridView(UnlockView):
    def __init__(self, model, canvas, xtiles=5, ytiles=5, tile_width=100,
        ...
        self.cursor = self.drawRect(self.xcenter - tile_width / 2,
                                    ...
                                    fill=True)
        cx,cy = canvas.center()
        self.femg_cursor = PygletTextLabel(UnlockState(True), canvas, '\u271b', cx, cy )
...
#                self.mark_target()
                self.assign_target()
        if state.femg is not None:
            self.femg_cursor.label.x = int(state.femg[0])
            self.femg_cursor.label.y = self.canvas.height - int(state.femg[1])
