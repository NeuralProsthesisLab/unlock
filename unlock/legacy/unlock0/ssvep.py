...
class SSVEP(UnlockApplication):
    ...
    def pause(self, duration):
        ...
        duration - number of seconds to pause
        """
        self.reset()
    ...
    def update(self, dt, decision, selection):
        ...
        if self._paused:
            ...
            if self._trial_time >= self.trial_length:
...
def generateCheckerboard(size, frequencies, duty_cycles, uneven, colors):
    ...
    frequencies - spatial frequencies in the x and y directions
    duty_cycles - ratio of 'on' to 'off' in the x and y directions
    uneven - is the last cycle only half-width or -height?
    colors - 'on' and 'off' rgb values
    """
    width = size[0]
    height = size[1]
