...
#        if decoder:
        self.h        = decoder      #info for HSD decoders
        self.mag      = mag
...
#        self.freqs    = freqs
        ...
        for i in range(numchan):
            if self.graph == 'pwr':
                ...
                pass
#                trace = zip(self.screenMapX(self.freqs),self.screenMapY(self.harmSums))
...
##                for i in range(10):
##                    pass
##                    self.harmSums[i] = self.h.harmSum(self.winsF[i,:],self.buf)
            self.index += length
...
#            self.cursor.vertices[::2] = [self.screen.x + self.screenMapX(self.index%self.numsamp) for i in self.cursor.vertices[::2]]
            for i in range(self.numchan):
                if self.graph == 'pwr':
                    ...
                    self.plots[i].updateData(self.screenMapY(self.harmSums))
