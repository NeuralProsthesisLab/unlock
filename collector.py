...
class VisualCues(UnlockApplication):
    name = "Visual Cue"
    icon = "robot.png"  
    def __init__(self, cues_list, screen, duration, trigger):
    ...
    def update(self, dt, decision, selection):
        ...
        if self.trial_time >= self.duration:
            self.trial_time = 0
            self.current = (self.current + 1) % len(self.cues)
...
class SampleCollector(UnlockApplication):
    ...
    def __init__(self):
        ...
        parser.add_option("-d", "--cue-duration", default='5', type=str, dest="duration", metavar="DURATION", help=duration_help)
        ...
        if options.cues != '':
            ...
            self.vc = VisualCues(options.cues.split(','), app_screen, self.duration, self.trigger)
