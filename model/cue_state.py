...
class RandomCueStateMachine(UnlockModel):
    ...
    def cue_state(self):
        return self.__transition__(self.indicator_state)
    ...
    def create(trials, cues, cue_duration, rest_duration, indication_duration, seed=42):
        ...
        indicate_state = CueState.create(Trigger.Indicate, indication_duration)
        state_machine = RandomCueStateMachine(random.Random(seed), trials, cue_states, rest_state, indicate_state)
        ...
        indicator_state.transition_fn = state_machine.indicate_state
        rest_state.start()
